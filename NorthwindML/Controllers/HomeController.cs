...
namespace NorthwindML.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult GenerateDatasets()
        {
            ...
            {
                IEnumerable<Order> ordersInCountry = db.Orders// filter by country to create different datasets
                    .Where(order => order.Customer.Country == country).Include(order => order.OrderDetails).AsEnumerable(); // switch to client-side
        ...
        public IActionResult Cart(int? id)
        {
            ...
            {
                model.Cart.Items = cartCookie.Split('-').Select(item => new CartItem
                {
