...
namespace Packt.Shared
{
    public class Northwind : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // define a one to many relationship
            modelBuilder.Entity<Employee>().HasMany(c => c.Orders).WithOne(c => c.Employee);

            modelBuilder.Entity<Product>().Property(c => c.ProductName).IsRequired().HasMaxLength(40);
            // define a many to one relationship
            modelBuilder.Entity<Product>().HasOne(p => p.Category).WithMany(s => s.Products);
            ...
            modelBuilder.Entity<Supplier>().HasMany(s => s.Products).WithOne(o => o.Supplier);
        }
    }
}
